"""
Modo Pitch — MMM econométrico rápido (Ridge con no-negatividad)  ·  v2
Cliente: Tigo-Millicom · Consumer Science & Analytics (CSA)

Novedades v2
  · Recomendador de hiperparámetros y de especificación (barrido univariado)
  · Puntos iniciales por canal + anclaje a un benchmark de contribución
  · Dummies: manuales, por fecha/rango/escalón/mes y SUGERIDAS por residuales
  · Alerta explícita cuando el R² no llega al umbral mínimo (50% por defecto)

Ejecuta:  streamlit run app_pitch_mmm_v2.py
"""
import io
import json
import base64
...
.csa-banner { background: linear-gradient(120deg,#00263A 0%,#003A5C 60%,#0088CE 140%);
...
h2, h3 { color:#00263A; }
div[data-testid="stMetric"] { background:#F4F8FB; border:1px solid #e3edf3; border-left:5px solid #0088CE;
...
table{{border-collapse:collapse;width:100%;font-size:.85rem;}} th{{background:#00263A;color:#fff;padding:6px 8px;text-align:left;}}
td{{padding:5px 8px;border-bottom:1px solid #e6edf1;}}
.cards{{display:flex;gap:12px;margin:10px 0;flex-wrap:wrap;}}
.card{{flex:1;min-width:150px;background:#fff;border-left:5px solid #0088CE;border-radius:8px;padding:12px 14px;}}
.card b{{font-size:1.4rem;color:#00263A;}} .foot{{color:#8aa0ad;font-size:.8rem;text-align:center;margin-top:26px;}}
...
def main():
    ...
    st.set_page_config(page_title="Modo Pitch MMM · Tigo-Millicom", page_icon="⚡", layout="wide")
    st.markdown(CSS, unsafe_allow_html=True)
    st.markdown(
        '<div class="csa-banner"><div class="csa-kicker">Consumer Science &amp; Analytics · Marketing Mix Modeling</div>'
        '<div class="csa-title">Modo Pitch — modelo econométrico rápido</div>'
        '<div class="csa-sub">Ridge con no-negatividad sobre adstock y saturación. Recomendador de '
        'hiperparámetros, anclaje a benchmark y dummies sugeridas por residuales.</div>'
    ...
    if archivo is None:
        st.info("👆 Sube un archivo con una columna de fecha y el resto numéricas para empezar.")
        st.stop()

    buf = io.BytesIO(archivo.getvalue())
    df = pd.read_excel(buf) if archivo.name.lower().endswith((".xlsx", ".xls")) else pd.read_csv(buf)
    ...
    with st.expander("⚙️  Configuración de campos", expanded=True):
        ...
        kpi_col = c2.selectbox("KPI a explicar", numeric)
        c3, c4 = st.columns(2)
        with c3:
            if st.checkbox("Elegir medios por patrón de texto", value=True):
                pat = st.text_input("Patrón para medios", "INVR")
                media_cols = [c for c in numeric if pat and pat.lower() in c.lower() and c != kpi_col]
                st.caption(f"{len(media_cols)} columnas coinciden")
            else:
                media_cols = st.multiselect("Medios (inversión)", [c for c in numeric if c != kpi_col])
        with c4:
            ctrl_cols = st.multiselect("Controles (precio, competencia, distribución…)",
                                       [c for c in numeric if c != kpi_col and c not in media_cols])
        valor_unitario = st.number_input(
            "Valor monetario de 1 unidad del KPI (0 = no aplica)", 0.0, 1e9, 0.0, step=1.0)

    if not media_cols:
        st.error("⚠️ Selecciona al menos un canal de inversión.")
        st.stop()

    work = df.copy()
    work[date_col] = pd.to_datetime(work[date_col], errors="coerce")
    d = (work[[date_col, kpi_col] + media_cols + ctrl_cols]
         .dropna(subset=[date_col]).sort_values(date_col).reset_index(drop=True))
    d[media_cols + ctrl_cols] = d[media_cols + ctrl_cols].fillna(0)
    d = d.dropna(subset=[kpi_col]).reset_index(drop=True)
    n = len(d)
    if n < 25:
        st.error("Se necesitan al menos 25 periodos para estimar algo confiable.")
        st.stop()
    ...
    st.session_state.setdefault("dummies", [])
    st.session_state.setdefault("reco", None)
    ...
    with st.expander("🧭  Recomendador de especificación e hiperparámetros", expanded=True):
        st.caption("Barre theta y alpha canal por canal contra el KPI limpio de tendencia y "
                   "estacionalidad, y propone punto inicial y rango. Es el sustituto práctico de los priors.")
        if st.button("🧭 Analizar y recomendar"):
            C_prev, _ = construir_controles(n, True, True, 2, float(periodo_def))
            C_prev = np.hstack([d[ctrl_cols].values.astype(float), C_prev]) if ctrl_cols else C_prev
            ...
            st.session_state["reco"] = {"esp": esp, "tabla": tab_r, "rangos": rangos_r,
                                        "inicios": inicios_r, "reactivos": reactivos}
            for c in media_cols:
                st.session_state[f"th_{c}"] = rangos_r[c]["theta"]
                st.session_state[f"al_{c}"] = rangos_r[c]["alpha"]
                st.session_state[f"th0_{c}"] = inicios_r[c]["theta"]
                st.session_state[f"al0_{c}"] = inicios_r[c]["alpha"]
            st.session_state["four_reco"] = esp["armonicos"]
            st.session_state["cuad_reco"] = esp["tendencia_cuadratica"]
            st.rerun()

        if st.session_state["reco"]:
            R0 = st.session_state["reco"]
            st.success(f"Estructura sugerida: {R0['esp']['armonicos']} armónico(s) de estacionalidad, "
                       ...
                       "Los rangos y puntos iniciales ya quedaron cargados abajo.")
            if R0.get("reactivos"):
                st.warning(
                    ...
                    "puede desenredar eso: o los anclas con un benchmark de contribución, o los sacas "
                    "del bloque de medios y los tratas como control.")
            st.dataframe(R0["tabla"], use_container_width=True)
    ...
    with st.expander("🎛️  Especificación del modelo", expanded=True):
        ...
        with e1:
            ...
            usar_tend = st.checkbox("Tendencia lineal", value=True)
            usar_cuad = st.checkbox("Tendencia cuadrática",
                                    value=bool(st.session_state.get("cuad_reco", True)))
            n_fourier = st.slider("Armónicos de estacionalidad", 0, 6,
                                  int(st.session_state.get("four_reco", 2)))
            periodo = st.number_input("Periodo estacional", 2.0, 400.0, float(periodo_def), 0.01)
        with e2:
            st.markdown("**Transformaciones**")
            tipo_sat = st.radio("Curva de saturación", ["potencia", "hill"], horizontal=True)
            L = st.slider("Memoria máxima del adstock (periodos)", 2, 24, 12)
        with e3:
            ...
            objetivo = st.selectbox("Qué optimizar",
                                    ["Mixto: R² ajustado + fuera de muestra (recomendado)",
                                     ...
                                     "R² fuera de muestra"])
            n_iter = st.slider("Iteraciones de búsqueda", 50, 2000, 400, 50)
            holdout = st.slider("Tamaño de cada ventana de validación", 0, max(int(n * 0.3), 1),
                                min(12, max(int(n * 0.15), 1)))
            pliegues = st.slider("Ventanas de validación (origen móvil)", 1, 5, 3)
            refinar = st.checkbox("Refinamiento local (afina el R²)", value=True)
            semilla = st.number_input("Semilla", 0, 10_000, 42, 1)

        b1, b2, b3 = st.columns(3)
        umbral_r2 = b1.slider("R² mínimo aceptable (alerta)", 0.0, 0.95, 0.50, 0.05)
        banda = (b2.number_input("Contribución mínima creíble %", 0.0, 100.0, 5.0, 1.0),
                 b3.number_input("Contribución máxima creíble %", 0.0, 100.0, 40.0, 1.0))

        st.markdown("**Rangos y puntos iniciales por canal**")
        st.caption("El punto inicial se evalúa siempre y es desde donde arranca el refinamiento. "
                   "El rango acota la búsqueda aleatoria.")
        rangos, inicio_th, inicio_al = {}, [], []
        for c in media_cols:
            r1, r2, r3, r4, r5 = st.columns([2.2, 2.6, 2.6, 1.3, 1.3])
            r1.markdown(f"`{c}`")
            th = r2.slider(f"rango theta {c}", 0.0, 0.95,
                           st.session_state.get(f"th_{c}", (0.0, 0.85)), 0.05,
                           key=f"th_{c}", label_visibility="collapsed")
            al = r3.slider(f"rango alpha {c}", 0.1, 1.0,
                           st.session_state.get(f"al_{c}", (0.25, 0.95)), 0.05,
                           key=f"al_{c}", label_visibility="collapsed")
            th0 = r4.number_input(f"θ0 {c}", 0.0, 0.95,
                                  float(st.session_state.get(f"th0_{c}", np.mean(th))), 0.05,
                                  key=f"th0_{c}", label_visibility="collapsed")
            al0 = r5.number_input(f"α0 {c}", 0.05, 1.0,
                                  float(st.session_state.get(f"al0_{c}", np.mean(al))), 0.05,
                                  key=f"al0_{c}", label_visibility="collapsed")
            rangos[c] = {"theta": th, "alpha": al}
            inicio_th.append(th0)
            inicio_al.append(al0)
        st.caption("Columnas: canal · rango theta · rango alpha · theta inicial · alpha inicial")

    # ---------------- benchmark ----------------
    with st.expander("🎯  Benchmark de contribución (opcional)", expanded=False):
        st.caption("Empuja la solución hacia contribuciones conocidas de modelos previos o del "
                   "benchmark de categoría. Es una penalización blanda: si el ajuste se degrada "
                   "mucho al forzarla, los datos están discrepando del benchmark.")
        peso_b = st.slider("Peso del benchmark (0 = solo R²)", 0.0, 3.0, 0.0, 0.1)
        usar_total = st.checkbox("Anclar la contribución TOTAL de medios")
        bench_total = st.number_input("Contribución total objetivo %", 0.0, 100.0, 20.0, 0.5) \
            if usar_total else None
        usar_canal = st.checkbox("Anclar por canal")
        bench_canal = {}
        if usar_canal:
            cols = st.columns(min(len(media_cols), 4))
            for j, c in enumerate(media_cols):
                with cols[j % len(cols)]:
                    v = st.number_input(f"{c} %", 0.0, 100.0, 0.0, 0.1, key=f"bch_{c}")
                    if v > 0:
                        bench_canal[j] = v
    ...
    # ---------------- dummies ----------------
    with st.expander(f"🏷️  Dummies aplicadas ({len(st.session_state['dummies'])})", expanded=False):
        binarias = [c for c in numeric if c not in media_cols + [kpi_col]
                    and set(pd.unique(d[c].dropna())) <= {0, 1, 0.0, 1.0}] if numeric else []
        if binarias:
            st.caption(f"Columnas 0/1 detectadas en tu base: {', '.join(binarias)} "
                       "(agrégalas como controles si quieres usarlas).")
        c1, c2, c3 = st.columns(3)
        with c1:
            f_punto = st.date_input("Dummy de una fecha puntual", value=None, key="d_punto")
            if st.button("Agregar fecha puntual") and f_punto:
                st.session_state["dummies"].append(
                    {"tipo": "punto", "fechas": [str(f_punto)], "nombre": f"D_pico_{f_punto:%Y%m%d}",
                     "motivo": "manual"})
                st.rerun()
        with c2:
            f_ini = st.date_input("Inicio de periodo", value=None, key="d_ini")
            f_fin = st.date_input("Fin de periodo", value=None, key="d_fin")
            if st.button("Agregar periodo") and f_ini and f_fin:
                st.session_state["dummies"].append(
                    {"tipo": "rango", "inicio": str(f_ini), "fin": str(f_fin),
                     "nombre": f"D_periodo_{f_ini:%Y%m%d}", "motivo": "manual"})
                st.rerun()
        with c3:
            f_esc = st.date_input("Escalón desde", value=None, key="d_esc")
            mes_sel = st.multiselect("Dummies de mes", list(range(1, 13)))
            cbot1, cbot2 = st.columns(2)
            if cbot1.button("Agregar escalón") and f_esc:
                st.session_state["dummies"].append(
                    {"tipo": "escalon", "desde": str(f_esc), "nombre": f"D_escalon_{f_esc:%Y%m%d}",
                     "motivo": "manual"})
                st.rerun()
            if cbot2.button("Agregar meses") and mes_sel:
                for m in mes_sel:
                    st.session_state["dummies"].append(
                        {"tipo": "mes", "mes": int(m), "nombre": f"D_mes_{int(m):02d}",
                         "motivo": "manual"})
                st.rerun()
        if st.session_state["dummies"]:
            st.dataframe(pd.DataFrame([{"dummy": x["nombre"], "tipo": x["tipo"],
                                        "motivo": x.get("motivo", "manual")}
                                       for x in st.session_state["dummies"]]),
                         use_container_width=True)
            if st.button("🗑️ Quitar todas las dummies"):
                st.session_state["dummies"] = []
                st.rerun()

    correr = st.button("⚡ Estimar modelo", type="primary")
    ...
    if correr:
        ...
        C_dum, nom_dum = construir_dummies(fechas, st.session_state["dummies"])
        C = np.hstack([x for x in (C_user, C_gen, C_dum) if x.size]) if (
            C_user.size or C_gen.size or C_dum.size) else np.zeros((n, 0))
        ...
        mejor, top = buscar(bus, rangos, n_iter, objetivo, banda, int(semilla), refinar,
                            ...
                            progreso=lambda p: barra.progress(min(p, 1.0), text="Explorando combinaciones…"))
        barra.empty()
        st.session_state["res"] = dict(
            mejor=mejor, top=top, bus=bus, medios=media_cols, nombres_ctrl=nombres_ctrl,
            y=y, mat_inv=mat_inv, fechas=fechas, valor=valor_unitario or None, banda=banda,
            umbral_r2=umbral_r2, bench_canal=bench_canal,
            config={"date_col": date_col, "kpi_col": kpi_col, "media_cols": media_cols,
                    ...
                    "benchmark": {"total": bench_total, "canal": {media_cols[j]: v
                                                                  for j, v in bench_canal.items()},
    ...
    mejor, bus, medios, y = R["mejor"], R["bus"], R["medios"], R["y"]
    aportes, base, yhat, X = descomponer(bus, mejor, medios, R["nombres_ctrl"], y)
    tabla = tabla_canales(medios, aportes, R["mat_inv"], y, mejor, R["valor"], R["bench_canal"])
    avisos = diagnosticar(mejor, medios, tabla, R["banda"], R["umbral_r2"],
                          len(st.session_state["dummies"]))

    t_fit, t_desc, t_dum, t_curv, t_cand, t_exp = st.tabs(
    ...
    with t_fit:
        ...
        m[2].metric("R² fuera de muestra (prom.)",
        ...
        f = fig_ajuste(R["fechas"], y, yhat); st.pyplot(f); plt.close(f)
    ...
    with t_desc:
        ...
        st.dataframe(pd.DataFrame({"variable": R["nombres_ctrl"],
                                   "coeficiente": np.round(mejor["beta"][k:], 3)}),
                     use_container_width=True)

    with t_dum:
        st.markdown("### Dummies sugeridas por los residuales")
        st.caption("Cada candidata se prueba en el modelo actual y se ordena por cuánto sube el R² "
                   "ajustado. Aplica de arriba hacia abajo y vuelve a estimar.")
        resid = y - yhat
        specs = sugerir_dummies(R["fechas"], resid)
        tabla_d, specs_ord = evaluar_sugerencias(R["fechas"], X, y, mejor["lam"], bus.pos_mask,
                                                 specs, bus.h, bus.pliegues, bus.k)
        if len(tabla_d) == 0:
            st.success("No hay residuales atípicos que justifiquen una dummy. El modelo ya está limpio.")
        else:
            z = np.abs(resid - np.median(resid))
            marcas = np.argsort(-z)[:6]
            f = fig_residuales(R["fechas"], y, yhat, marcas); st.pyplot(f); plt.close(f)
            ...
            cA, cB = st.columns(2)
            if cA.button("➕ Aplicar seleccionadas"):
                actuales = {x["nombre"] for x in st.session_state["dummies"]}
                for nom, s in zip(tabla_d["dummy"], specs_ord):
                    if nom in elegidas and nom not in actuales:
                        st.session_state["dummies"].append(s)
                st.rerun()
            cB.info("Después de aplicarlas, vuelve arriba y pulsa **Estimar modelo** para reajustar "
                    "los hiperparámetros con las dummies dentro.")
            st.caption("**ΔR2_fuera** dice si la dummy explica algo o solo absorbe una observación "
                       "incómoda. **Δmedios_pp** dice cuánto mueve la contribución total de medios: "
                       "una dummy que sube el R² pero desplaza la descomposición más de 4 puntos está "
                       "reasignando crédito, no limpiando ruido. Solo las marcadas como *explica* se "
                       "seleccionan por defecto; si aplicas varias, ancla la descomposición con el "
    ...
    with t_curv:
        ...
        st.dataframe(pd.DataFrame(filas).sort_values("KPI_marginal_por_1000", ascending=False),
                     use_container_width=True)
    ...
    with t_cand:
        ...
        st.dataframe(pd.DataFrame([{
            ...
            "score": round(c["score"], 4)} for c in R["top"]]), use_container_width=True)

    with t_exp:
        meta = {"fecha": datetime.now().strftime("%Y-%m-%d %H:%M"), "filas": len(y),
                "medios": len(medios),
                "periodo": f"{pd.Timestamp(R['fechas'].iloc[0]):%Y-%m-%d} a "
                           f"{pd.Timestamp(R['fechas'].iloc[-1]):%Y-%m-%d}"}
        figs_b64 = [("KPI real vs estimado", fig_to_b64(fig_ajuste(R["fechas"], y, yhat))),
        ...
        codigo = export_json(R["config"], mejor, medios, tabla, st.session_state["dummies"])
        st.code(codigo, language="python")
        st.download_button("⬇️ Descargar config_pitch.py", codigo,
                           file_name="config_pitch.py", mime="text/x-python")
        st.download_button("⬇️ Tabla de canales (CSV)", tabla.to_csv(index=False),
                           file_name="canales_pitch.csv", mime="text/csv")
